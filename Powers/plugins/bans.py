...
async def tban_usr(c: Gojo, m: Message):
    ...
    try:
        ...
        admins_group = {i[0] for i in (await admin_cache_reload(m, "ban"))}
...
async def kick_usr(c: Gojo, m: Message):
    ...
    try:
        ...
        admins_group = {i[0] for i in (await admin_cache_reload(m, "kick"))}
...
async def dban_usr(c: Gojo, m: Message):
    ...
    try:
        await m.reply_to_message.delete()
        await m.chat.ban_member(user_id)
        target = m.reply_to_message.from_user.mention if m.reply_to_message.from_user else user_first_name
        txt = f"{m.from_user.mention} banned {target} in <b>{m.chat.title}</b>!"
        if reason:
            txt += f"\n<b>Reason</b>: {reason}"
        keyboard = InlineKeyboardMarkup(
            [
        ...
        try:
            ...
            await c.send_message(MESSAGE_DUMP, f"#REMOVE from BAN_GIFS\n{animm}")
    except ChatAdminRequired:
        await m.reply_text(text="I'm not admin or I don't have rights.")
    except PeerIdInvalid:
        await m.reply_text(
...
async def unbanbutton(c: Gojo, q: CallbackQuery):
    ...
    elif not (user.privileges and user.privileges.can_restrict_members) and q.from_user.id != OWNER_ID:
        await q.answer(
            "You don't have enough permission to do this!\nStay in your limits!",
            show_alert=True,
        )
...
async def kickme(c: Gojo, m: Message):
    ...
    try:
        ...
        if "USER_ADMIN_INVALID" in str(ef):
            await m.reply_text("Looks like I can't kick you (⊙_⊙)")
            return
        elif "CHAT_ADMIN_REQUIRED" in str(ef):
