...
from Powers.utils.captcha_helper import genrator, get_image_captcha, get_qr_captcha
...
async def captcha_codes_check(c: Gojo, q: CallbackQuery):
    ...
    if code_ == code:
        ...
        try:
            ...
            await q.message.reply_text(f"Unable to unmute {q.from_user.mention}")
        ...
        if not tries:
            ...
            keyboard = ikm([[IKB("Unban", callback_data=f"unban_={user}")]])
...
async def on_chat_members_updatess(c: Gojo, m: Message):
    ...
    for user in users:
        ...
        try:
            status = (await m.chat.get_member(user.id)).status
        ...
        if not is_already:
            ...
            cap = f"Please {user.mention} choose the correct code below\nYou have three tries.\nTries left: 3"
            ...
            kb = ikm([[IKB("Click here to verify", url=mess.link)]])

            await c.send_message(
                chat,
                f"{user.mention} your verification is already pending",
                reply_markup=kb
            )
