...
async def wiki(_, m: Message):
    ...
    search = m.text.split(None, 1)[1].strip()
    try:
        res = summary(search)
...
async def get_lyrics(_, m: Message):
    ...
    song = ""
    if not query:
        await m.reply_text("You haven't specified which song to look for!")
        return
    song_name = query
...
async def id_info(c: Gojo, m: Message):
    ...
    try:
        ...
        return
    if user_id:
        if m.reply_to_message and m.reply_to_message.forward_origin and getattr(m.reply_to_message.forward_origin, "sender_user", None):
...
Forwarder - {fwd_sender} (<code>{fwd_id}</code>)""",
      ...
      Peer ID invalid, I haven't seen this user anywhere earlier, maybe username would help to know them!"""
        ...
        if m.reply_to_message:
            if m.forward_origin and getattr(m.forward_origin, "sender_user", None):
                text += f"Forwarded from user ID <code>{m.forward_origin.sender_user.id}</code>."
            elif m.forward_origin and getattr(m.forward_origin, "chat", None):
                text += f"Forwarded from user ID <code>{m.forward_origin.chat.id}</code>."
...
async def github(_, m: Message):
    ...
    if avtar:
        return await m.reply_photo(photo=avtar, caption=REPLY)
...
async def paste_func(_, message: Message):
    ...
    if not r and len(message.text.split()) < 2:
        return await m.edit("Give text or reply to message")
...
async def reporting_query(c: Gojo, m: Message):
    ...
    ppost = z.link
    await c.send_message(OWNER_ID, f"New bug report\n{ppost}", link_preview_options=LinkPreviewOptions(is_disabled=True))
    return
