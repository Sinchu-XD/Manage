...
on_key = ["on", "start", "enable"]
off_key = ["off", "end", "disable", "stop"]
...
async def flood_action(c: Gojo, m: Message):
    ...
    status = bot.status
    if status not in [CMS.OWNER, CMS.ADMINISTRATOR] or not getattr(bot.privileges, "can_restrict_members", False):
        return await m.reply_text("Give me permission to restrict members first")
...
async def callbacks(c: Gojo, q: CallbackQuery):
    ...
    is_flood = Flood.is_chat(c_id)
    if not is_flood:
        await q.answer("Flood protection is disabled", show_alert=True)
        return
    saction = is_flood[2]
    slimit = is_flood[0]
    swithin = is_flood[1]
