...
async def remove_sticker_from_pack(c: Gojo, m: Message):
    ...
    if not sticker_set:
        await to_modify.edit_text("This sticker is not part of your pack.")
        return
    ...
    try:
        ...
        await to_modify.edit_text(
            f"Successfully removed [sticker]({m.reply_to_message.link}) "
            f"from **{sticker_set.set.title}**",
        ...
        await to_modify.edit_text(
            f"Failed to remove sticker due to:\n`{e}`\nPlease report this bug using `/bug`"
        )
...
async def memify_it(c: Gojo, m: Message):
    ...
    if not (
        ...
        or rep_to.animation
        or rep_to.video
        or (rep_to.document and rep_to.document.mime_type and
            rep_to.document.mime_type.split("/")[0] in ["image", "video"])
    ):
        await m.reply_text("Reply to sticker/photo/gif/video.")
        return

    kb = IKM([[IKB("You might like", url="https://t.me/ProNovaSupport")]])
    ...
    name = f"@memesofdank_{m.id}"
    path = await rep_to.download(name)

    try:

        if rep_to.sticker and rep_to.sticker.is_animated:
            path = await tgs_to_gif(path, tgs=True)

        elif rep_to.sticker and rep_to.sticker.is_video:
            path = await webm_to_gif(path)

        elif rep_to.video or rep_to.animation:
            path = await webm_to_gif(path)

    except Exception as e:
        await x.edit(f"Conversion failed: {e}")
        return
    ...
    try:
        ...
        await x.edit(f"Memify failed: {e}")
        return

    await x.delete()

    # MEDIA TYPE BASED OUTPUT
    if rep_to.photo:
        await m.reply_photo(output[0], reply_markup=kb)

    elif rep_to.sticker:
        await m.reply_sticker(output[1], reply_markup=kb)

    elif rep_to.animation:
        await m.reply_animation(output[0], reply_markup=kb)

    elif rep_to.video:
        await m.reply_video(output[0], reply_markup=kb)

    else:
        await m.reply_document(output[0], reply_markup=kb)
...
async def quote_the_msg(_, m: Message):
    if not m.reply_to_message:
        ...
        return
    if not m.reply_to_message.from_user or not m.reply_to_message.text:
        await m.reply_text("I can only quote text messages from users.")
        return
