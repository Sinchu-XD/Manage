...
async def view_filters(_, m: Message):
    ...
    return await m.reply_text(filters_chat, link_preview_options=LinkPreviewOptions(is_disabled=True))
...
async def stop_filter(_, m: Message):
    ...
    cmd = m.text.split(None, 1)[1].lower()

    chat_filters = db.get_all_filters(m.chat.id) or []
...
async def filters_watcher(c: Gojo, m: Message):
    ...
    for trigger in sorted(actual_filters, key=len, reverse=True):
        pattern = rf"(?:^|\W){re_escape(trigger)}(?:$|\W)"
