...
async def markdownhelp(_, m: Message):
    await m.reply_photo(
        ...
        reply_markup=(await gen_formatting_kb()),
    )
    return
...
@Gojo.on_callback_query(filters.regex(r"^next_format_\d+$"))
...
ProNoVa supports a large number of formatting options to make your messages more expressive. Take a look by clicking the buttons below!""",
        reply_markup=(await gen_formatting_kb()),
    )
    await q.answer()
    return
