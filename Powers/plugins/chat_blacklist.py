...
@Gojo.on_message(command(["rmblchat", "unblchat"], dev_cmd=True))
async def unblacklist_chat(c: Gojo, m: Message):
    ...
    if len(m.text.split()) >= 2:
        ...
        await replymsg.edit_text(
            f"Removed the following chats from Blacklist.\n<code>{', '.join(chat_ids)}</code>.",
        )
...
@Gojo.on_message(command(["blchatlist", "blchats"], dev_cmd=True))
async def list_blacklist_chats(_, m: Message):
    ...
    if bl_chats:
        ...
        txt = "These Chats are Blacklisted:\n" + "\n".join(
            f"<code>{i}</code>" for i in bl_chats
        )
...
Dev commands:
• /blchat [chat id or username]: Add chats to blacklist
• /rmblchat [chat id or username]: Remove chats from blacklist
