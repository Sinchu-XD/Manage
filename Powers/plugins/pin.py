    ...
    try:
        if m.reply_to_message:
            ...
            if not chat.pinned_message:
                await m.reply_text("There is no pinned message!")
                return
            m_id = chat.pinned_message.id
...
async def anti_channel_pin(_, m: Message):
    ...
    if len(m.text.split()) == 2:
        if m.command[1] in ("yes", "on", "true"):
            ...
            msg = "Turned on AntiChannelPin, now all message pinned by channel will be unpinned automatically!"
            ...
            await m.reply_text(
                text="Please check help on how to use this command."
            )
            return
...
async def perma_pin(_, m: Message):
    if m.reply_to_message or len(m.command) > 1:
        if m.reply_to_message:
