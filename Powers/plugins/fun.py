...
async def fun_slap(c: Gojo, m: Message):
    ...
    curr_user = escape(m.from_user.first_name) if m.from_user else "Someone"
    user = m.reply_to_message.from_user if m.reply_to_message else m.from_user
    if not user:
        await m.reply_text("I can't slap anonymous users.")
        return
    user_id = user.id
    ...
    if user_id != m.from_user.id:
        user1 = curr_user
        user2 = escape(user.first_name) if user.first_name else "Deleted User"
...
async def insult(c: Gojo, m: Message):
    ...
    if not user:
        await m.reply_text("I can't insult anonymous users.")
        return
