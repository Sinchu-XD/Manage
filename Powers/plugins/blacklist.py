...
async def add_blacklist(_, m: Message):
    ...
    if len(m.text.split()) < 2:
        await m.reply_text("Please check help on how to use this command.")
        return
    ...
    await m.reply_text(
        f"Added {trigger} in blacklist words!"
...
@Gojo.on_message(command(["blwarning", "blreason", "blacklistreason"]) & restrict_filter)
...
@Gojo.on_message(command(["rmblacklist", "unblacklist"]) & restrict_filter)
async def rm_blacklist(_, m: Message):
    ...
    await m.reply_text(
        f"Removed <b>{bl_words}</b> from blacklist words!"
        + (f"\n{rep_text}" if rep_text else "")
    )
...
@Gojo.on_message(command(["blaction", "blacklistaction", "blacklistmode"]) & restrict_filter)
async def set_bl_action(_, m: Message):
    ...
    if len(m.text.split()) == 2:
        ...
        await m.reply_text(
            f"Set action for blacklist for this to <b>{action}</b>"
        )
        ...
        await m.reply_text(
            f"""The current action for blacklists in this chat is <i><b>{action}</b></i>
...
@Gojo.on_message(command("rmallblacklist") & owner_filter)
async def rm_allblacklist(_, m: Message):
    ...
    if not all_bls:
        await m.reply_text("No blacklists in this chat")
        return
...
async def rm_allbl_callback(_, q: CallbackQuery):
    ...
    if user_status != CMS.OWNER:
        await q.answer(
            "Only chat owner can clear blacklists!",
            show_alert=True,
        )
        return
