...
async def unlock_perm(c: Gojo, m: Message):
    ...
    if unlock_type == "msg":
        ...
        if not curr:
            await m.reply_text("User already can add bots in the chat")
            return
...
async def lock_del_mess(c: Gojo, m: Message):
    ...
    # ignore channel / anonymous messages
    if not m.from_user:
        member = None
    else:
        try:
    ...
    # ignore admins
    if member and member.status in (CMS.ADMINISTRATOR, CMS.OWNER):
        return

    # anti channel messages
    if (
        chat_locks["anti_channel"]
        and m.sender_chat
        and not (m.forward_origin and getattr(m.forward_origin, "chat", None))
        and not (m.forward_origin and getattr(m.forward_origin, "sender_user", None))
    ):
    ...
    # anti forward
        ...
        elif chat_locks["anti_fwd"]["user"] and not getattr(m.forward_origin, "chat", None):
            await delete_messages(c, m)
            return

        elif chat_locks["anti_fwd"]["chat"] and getattr(m.forward_origin, "chat", None):
            await delete_messages(c, m)
            return
