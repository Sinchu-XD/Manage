...
async def disableit(_, m: Message):
    ...
    cmd = m.text.split(maxsplit=1)[1].strip().lower()

    disable_cmd_keys = sorted(
        k
        for j in (HELP_COMMANDS[i].get("disablable", []) for i in HELP_COMMANDS)
        for k in j
    )
    ...
    if cmd in disable_list:
        return await m.reply_text("It's already disabled!")

    if cmd in disable_cmd_keys:
        db.add_disable(cmd)
        return await m.reply_text(f"Disabled {cmd}!")
...
async def enableit(_, m: Message):
    ...
    if cmd not in disable_list:
        return await m.reply_text("It's not disabled!")

    db.remove_disabled(cmd)

    return await m.reply_text(f"Enabled {cmd}!")
...
async def disabling(_, m: Message):
    ...
    tes = "List of commands that can be disabled:\n" + "\n".join(
...
async def enablealll(_, q: CallbackQuery):
    ...
    db.rm_all_disabled()
    await q.answer("Enabled all!", show_alert=True)
