...
• Download: {download}
"""
    await m.reply_text(txt, link_preview_options=LinkPreviewOptions(is_disabled=True))
    return


@Gojo.on_message(command("logs", owner_cmd=True))
async def send_log(c: Gojo, m: Message):
    ...
    await m.reply_document(document=LOGFILE)

    await replymsg.delete()
    return

@Gojo.on_message(command("neofetch", owner_cmd=True))
...
@Gojo.on_message(command(["eval", "py"], owner_cmd=True))
...
@Gojo.on_message(command(["exec", "sh"], owner_cmd=True))
...
async def uptime(_, m: Message):
    ...
    await m.reply_text(text=f"<b>Uptime:</b> <code>{up}</code>")
    return
