...
async def get_private_note(c: Gojo, m: Message, help_option: str):
    ...
    if len(help_lst) == 2:
        ...
        chat_data = Chats.get_chat_info(chat_id)
        chat_title = chat_data["chat_name"] if chat_data else "this chat"
        ...
        await m.reply_text(rply, link_preview_options=LinkPreviewOptions(is_disabled=True))
    ...
    if not getnotes:
        await m.reply_text("Note does not exist")
        return
    ...
    try:
        ...
        splitter = "%%%"
        note_value = getnotes.get("note_value") or ""
        note_reply = note_value.split(splitter) if note_value else [""]
    ...
    if msgtype == Types.TEXT:
        ...
        if button:
            ...
            await m.reply_text(teks, link_preview_options=LinkPreviewOptions(is_disabled=True))
