...
async def extract_time(m: Message, time_val: str):
    ...
    await m.reply(
        f"Invalid time type specified. Needed m, h, or d. got: {time_val[-1]}",
    )
    return ""
...
async def escape_mentions_using_curly_brackets(
    ...
    if m.chat.type in [
        ...
        chat_name = escape(m.chat.title or "")
    else:
        chat_name = escape(m.from_user.first_name or "")
    ...
    if teks:
        ...
        teks = teks.format(
            ...
            last=escape(last_name if last_name else first_name),
            mention=m.from_user.mention,
            username=(
                "@" + (await escape_markdown(m.from_user.username))
