...
class Config:
    ...
    OWNER_ID = int(config("OWNER_ID", default=6444277321))
    ...
    DB_NAME = config("DB_NAME", default="ProAbhi")
    BDB_URI = config("BDB_URI", default=None)
    NO_LOAD = config("NO_LOAD", default="").split()
    PREFIX_HANDLER = config("PREFIX_HANDLER", default="/").split()
    SUPPORT_GROUP = config("SUPPORT_GROUP", default="PronovaSupport")
    SUPPORT_CHANNEL = config("SUPPORT_CHANNEL", default="ProNovaUpdates")
...
class Development(Config):
    """Development config. Keep secrets in .env/environment, never in source."""
